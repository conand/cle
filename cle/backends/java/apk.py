...
class Apk(Soot):
    ...
    def __init__(self, apk_path, entry_point=None, android_sdk=None, supported_jni_archs=None,
        ...
        l.debug("Loading APK from %s ...", apk_path)
    ...
    def _extract_jni_libs(apk_path, supported_jni_archs):
        ...
        :return: Name of all extracted JNI libs together with the path to dir used for extracting.
        :rtype: tuple
        """
        l.debug("Trying to parse JNI libs from the APK")
        with ZipFile(apk_path) as apk:
            ...
            l.debug("Found JNI arch(s) %s. Picking %s.", ", ".join(available_jni_archs), jni_arch)
            ...
            l.debug("Extracted lib(s) to %s", jni_libs_ld_path)
    ...
    def is_compatible(stream):
        ...
        if not 'AndroidManifest.xml' in filelist:
            return False
        if not 'classes.dex' in filelist:
            return False
        return True
